...
# x is mcts vs o is alpha beta and return avg time to move and result 
def mcts_vs_ab(game: TicTacToe3Pieces, ai :alpha_beta , mc :mcts , ai_depth=5, mc_iter=1000 ,match = 10):
    ...
    for zzz in range(match):
        ...
        while(game.win_play(board)==0):
            ...
            game.print_board()
            if(game.win_play(board)== -1):
                print("O is win in raond (alpha beta)" , i)
                results['P2_Wins'] +=1
                break
            i +=1
...
# o is mcts vs x is alpha beta and return avg time to move and result 

def ab_vs_mcts(game: TicTacToe3Pieces, ai :alpha_beta , mc :mcts , ai_depth=5, mc_iter=1000 ,match = 10):
    results = {"P1_Wins": 0, "P2_Wins": 0 , 'Draws' :0}
    time_P1 = []
    time_P2 = []
    ...
    for zzz in range(match):
        ...
        while(game.win_play(board)==0):
            ...
            # X player in mcts algorithm
            start = time.time()
            mc.search(game , board , 1 , que_x , que_y ,mc_iter)
            x = ai.all_alpha_beta_y(board , game , ai_depth, -float('inf') , float('inf') , que_x , que_y)[1]
            end = time.time()
            ...
            game.print_board()
            if(game.win_play(board)==1):
                print("X is win raond (alpha beta)" , i+1)
                results['P1_Wins'] +=1
                break
            

            start = time.time()
            o = mc.search(game , board , 1 , que_x , que_y ,mc_iter)
            end = time.time()
            ...
            game.print_board()
            if(game.win_play(board)== -1):
                print("O is win in raond (mcts)" , i)
                results['P2_Wins'] +=1
                break
            i +=1
    ...
    return {
        "results": results,
        "P1_time_ms": avg_P1*1000,
        "p2_time_ms": avg_P2*1000,
        "total time to one match S" : avg_P1*i+avg_P2*i,
        "one move x , o ms" :(time_P1[0]*1000, time_P2[0]*1000)
    } 
...
test =ab_vs_mcts(game , ai ,mc ,8, 500 ,10)

# test = ab_vs_ab(game , ai ,5,5)

# test = mctc_vs_mcts(game , mc ,200 ,1000,10)
